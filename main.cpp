...
void set_values(Board& board)
{
    ...
    std::cout << "\ninserire la probabilità di morte di una cellula infetta fra 0 e 1: ";
    std::cin >> Board::mortality_rate;

    loop_of_failure(Board::mortality_rate, 1);

    int n_cells;
    std::cout << "\ninserire il numero di cellule suscettibili nella griglia, non maggiore della metà delle celle nella board: ";
    std::cin >> n_cells;
    loop_of_failure(n_cells, board.get_size() * board.get_size() / 2);

    board.fill_board(n_cells, SUSCEPTIBLE);

    std::cout << "\ninserire il numero di cellule infette nella griglia, non maggiore della lunghezza del lato della griglia: ";
    std::cin >> n_cells;
