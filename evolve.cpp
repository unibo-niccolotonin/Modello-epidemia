...
static std::uniform_real_distribution<double> distribution_for_probability(0, 1);
...
void infect_radius(int row, int column, Board const& board, Board& new_board)
{
    for (int i = 0; i < (2 * Board::infection_radius + 1); i++)
    {
        for (int j = 0; j < (2 * Board::infection_radius + 1); j++)
        {
            ...
            {
                double random = random_generation::distribution_for_probability(random_generation::device);
...
void update_infected_cells(int row, int column, Board const& board, Board& new_board)
{
    ...
    {
        double nrand = random_generation::distribution_for_probability(random_generation::device);
...
void update_movement(int row, int column, Board const& board, Board& new_board)
{
    ...
    while (failure_counter != 1) // Se si mette un valore maggiore di uno il movimento appare più casuale. La performance del programma però ne risente quando ci sono molte cellule
    {
        ...
        new_board(row, column).vx = random_generation::velocity_distribution(random_generation::device);
        new_board(row, column).vy = random_generation::velocity_distribution(random_generation::device);
